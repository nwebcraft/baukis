...
describe Staff::AccountsController do
  describe '#update' do
    ...
    example 'email属性を変更する' do
      ...
      patch :update, staff_member: params_hash
      staff_member.reload
    ...
    example '例外ActionController::ParameterMissingが発生' do
      bypass_rescue
      expect { patch :update }.to raise_error(ActionController::ParameterMissing)
    end

    example 'end_dateの値は更新できない' do
      params_hash.merge!(end_date: Date.tomorrow)
      expect { patch :update, staff_member: params_hash }.not_to change { staff_member.end_date }
    end
  end
end
